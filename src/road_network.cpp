...
FlatCutIndex::FlatCutIndex(const CutIndex &ci)
{
    ...
    size_t data_size = distance_offset + ci.distances.size() * sizeof(distance_t) + ci.paths.size() * sizeof(NodeID);
...
CHNeighbor::CHNeighbor(NodeID neighbor, NodeID triangle_node, distance_t distance) : neighbor(neighbor), triangle_node(triangle_node), distance(distance)
{
}

QNode::QNode(NodeID anc_node, NodeID pre_anc_node, distance_t distance) : anc_node(anc_node), pre_anc_node(pre_anc_node), distance(distance)
{
}
...
void Graph::customise_shortcut_graph(ContractionHierarchy &ch, vector<Edge> &edges) {
...
#ifdef MULTI_THREAD_DISTANCES
    vector<thread> threads;
    auto chp = [this](ContractionHierarchy &ch, util::par_bucket_list<NodeID, MULTI_THREAD_DISTANCES> &que, size_t id) 
    {
        NodeID x;
        while (que.next(x, id)) 
	{
...
void Graph::customise_tree_labelling(ContractionHierarchy &ch, ContractionIndex &tcl) {

#ifdef MULTI_THREAD_DISTANCES
    vector<thread> threads;
    auto tclp = [this](ContractionHierarchy &ch, ContractionIndex &tcl, util::par_bucket_list<NodeID, MULTI_THREAD_DISTANCES> &que, size_t id) 
    {
        NodeID x;
        while (que.next(x, id)) 
	{
...
void Graph::get_anc_dist(ContractionHierarchy &ch, ContractionIndex &tcl, NodeID v, uint16_t h_max, vector<QNode> &result)
{
    ...
    for (uint16_t h = v_height; h > 0; h--) 
    {
        ...
        if (ci.label_count() == 0) 
	{
            for (CHNeighbor &n : ch.nodes[current.anc_node].up_neighbors) 
	    {
...
void Graph::find_shortcut(ContractionHierarchy &ch, NodeID prev, NodeID current, CHNeighbor &n) {

    if(ch.nodes[prev].dist_index < ch.nodes[current].dist_index) swap(prev, current);
    for(CHNeighbor &x: ch.nodes[prev].up_neighbors) 
    {
...
void Graph::find_shortcut(ContractionHierarchy &ch, NodeID prev, uint16_t current, CHNeighbor &n) {
...
// appends path from w (exclusive) to v (inclusive)
void Graph::unpack_valley_path(ContractionHierarchy &ch, NodeID v, uint16_t w, std::vector<NodeID> &result)
{
    vector<NodeID> stack; 
...
// appends path from w (exclusive) to v (inclusive)
void Graph::unpack_valley_path(ContractionHierarchy &ch, NodeID v, NodeID w, std::vector<NodeID> &result)
{
    vector<NodeID> stack; stack.push_back(v);
    NodeID current = w; CHNeighbor shortcut(NO_NODE, NO_NODE, infinity);
    while (!stack.empty())
    {
        NodeID prev = stack.back();
        find_shortcut(ch, prev, current, shortcut);
...
void Graph::get_anc_dist(ContractionHierarchy &ch, NodeID v, std::vector<QNode> &result)
{   
    uint16_t v_height = ch.nodes[v].dist_index;
    result.resize(v_height + 1, QNode(NO_NODE, NO_NODE, infinity));
    result[v_height] = QNode(v, v, 0);
...
// appends path from w (exclusive) to v (inclusive)
void Graph::path_from_anc(ContractionHierarchy &ch, NodeID v, NodeID w, std::vector<QNode> &anc_dist, std::vector<NodeID> &result)
{
    NodeID current = w; size_t i = result.size();
    while (current != v)
    {
        NodeID prev = anc_dist[ch.nodes[current].dist_index].pre_anc_node;
        // unpack path segment from current to prev
...
vector<NodeID> Graph::query(ContractionHierarchy &ch, NodeID v, NodeID w)
{
    std::vector<QNode> v_anc, w_anc;
    get_anc_dist(ch, v, v_anc);
    get_anc_dist(ch, w, w_anc);

    distance_t min_dist = infinity; NodeID hub;
...
vector<NodeID> Graph::query(ContractionHierarchy &ch, ContractionIndex &tcl, NodeID v, NodeID w)
{
